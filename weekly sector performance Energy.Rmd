...
read_fin_data <- function(tickers, 
  ...
  df <- df %>% filter(Index <= end_date) %>% 
    ...
    mutate(adj_return = round((Close / first(Close) - 1)*100, 1))
  return(df)
}
...
```{r energy_3months_return, echo = FALSE, message = FALSE, warning=FALSE, out.width = 900}
df <- df_ticker %>% 
  mutate(price_data = map(ticker, function(.x) read_fin_data(.x, start_date = today() - 63))) %>% 
  unnest()

ggplot(df, aes(x = Index, y = adj_return, color = ticker)) + 
  geom_line() + 
  #scale_x_datetime() + 
  scale_x_bd(business.dates=df$Index, max.major.breaks = 10, labels=date_format("%d '%b"), expand = c(0,0.3)) + 
  ylab(label = "Adjusted return") + 
  ggtitle("3 months ETF return") + 
...
```{r energy_12months_return, echo = FALSE, message = FALSE, warning=FALSE, out.width = 900}
df <- df_ticker %>% 
...
ggplot(df, aes(x = Index, y = adj_return, color = ticker)) + 
  geom_line() + 
  #scale_x_datetime() + 
  scale_x_bd(business.dates=df$Index, max.major.breaks = 15, labels=date_format("%d '%b"), expand = c(0,0.3)) + 
  ylab(label = "Adjusted return") + 
  ggtitle("12 months ETF return") + 
  theme(legend.title = element_blank(),
        axis.title.x = element_blank(), 
        plot.title = element_text(hjust=0.5, face = "bold"), 
        panel.background = element_rect(fill = "black"), 
        ...
        panel.grid.major.y = element_line(color = "Gray 65", linetype = "solid", size = 0.2), 
        panel.grid.minor.y = element_line(color = "Gray 65", linetype = "dotted", size = 0.15),
        axis.text.x = element_text(angle = 60, vjust = 0.5))
...
ggplot(df, aes(x = Index, y = adj_return, color = ticker)) + 
  ...
  theme(legend.title = element_blank(),
        ...
        plot.background = element_rect(fill = "Gray 75"), 
        legend.key = element_rect(fill = "Gray 75"), 
        legend.background = element_rect(fill = "Gray 65"), 
        panel.grid.major.x = element_line(color = "white", linetype = "dotted", size = 0.2), 
        panel.grid.major.y = element_line(color = "Gray 65", linetype = "solid", size = 0.2), 
        panel.grid.minor.y = element_line(color = "Gray 65", linetype = "dotted", size = 0.15),
        axis.text.x = element_text(angle = 60, vjust = 0.5))
